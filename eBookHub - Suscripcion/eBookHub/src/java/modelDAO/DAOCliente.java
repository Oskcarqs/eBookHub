...
public class DAOCliente {
    ...
    public boolean add(Cliente c) {
        String sql = "insert into Cliente (nombreCliente, apellidoCliente, fechaDeNacimientoCliente, direccionCliente, telefonoCliente,imgCLiente, nickNameUsuario,emailUsuario,passwordUsuario) values(?,?,?,?,?,?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public boolean edit(Cliente c) {
        String sql = "update Cliente set nombreCliente=?, apellidoCliente=?, fechaDeNacimientoCliente=?, direccionCliente=?, telefonoCliente=?,passwordUsuario=?,imgCLiente=?,nickNameUsuario=?, emailUsuario=? where idCliente=?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setString(1, c.getNombreCliente());
    ...
    public boolean editTipoSuscripcion(Cliente c) {
        String sql = "update Cliente set idTipoSuscripcion = ? where idCliente = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, c.getIdTipoSuscripcion());
